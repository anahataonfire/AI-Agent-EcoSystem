...
class EdgeCalculator:
    ...
    def __init__(
        ...
        self.min_edge = min_edge or EDGE_CONFIG["min_edge_threshold"]
        self.high_edge = high_edge or EDGE_CONFIG["high_confidence_edge"]
        self.std_dev_24hr = std_dev_24hr or EDGE_CONFIG["std_dev_24hr"]
        self.std_dev_48hr = std_dev_48hr or EDGE_CONFIG["std_dev_48hr"]
    
    def _get_std_dev(self, hours_remaining: float) -> float:
        """Get appropriate std_dev based on time to resolution."""
        if hours_remaining <= 24:
            return self.std_dev_24hr
        elif hours_remaining <= 48:
            # Linear interpolation
            ratio = (hours_remaining - 24) / 24
            return self.std_dev_24hr + ratio * (self.std_dev_48hr - self.std_dev_24hr)
        else:
            return self.std_dev_48hr * 1.5  # Even more uncertainty
    ...
    def find_opportunities(
        ...
        for market in markets:
            # Get forecast for this city and date
            forecast = forecasts.get(market.city_key)
            if not forecast:
                logger.debug(f"No forecast for {market.city_key}")
                continue
